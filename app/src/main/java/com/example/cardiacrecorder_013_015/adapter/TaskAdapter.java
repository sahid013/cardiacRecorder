package com.example.cardiacrecorder_013_015.adapter;

import static com.example.cardiacrecorder_013_015.R.*;
...
public class TaskAdapter extends RecyclerView.Adapter<TaskAdapter.TaskViewHolder> {
    ...
    popUpScreenForAddingRecord.setRefreshListener setRefreshListener;

    /**
     *
     * @param context
     * @param taskList list of the task
     * @param setRefreshListener sets refresh listener
     */
    public TaskAdapter(MainActivity context, List<Task> taskList, popUpScreenForAddingRecord.setRefreshListener setRefreshListener) {
    ...
    public void onBindViewHolder(@NonNull TaskViewHolder holder, int position) {
        ...
        int systolicPress =Integer.parseInt(task.getSystolic_pressure());
        int diastolicPress =Integer.parseInt(task.getDiastolic_pressure());
        if((systolicPress>=90 && systolicPress<=140) && (diastolicPress>=60 && diastolicPress<=90))
        {
            holder.image.setImageResource(drawable.ic_goodhealth);
        }
        else {
            holder.image.setImageResource(drawable.ic_warning);
        }
    ...
    //This section is for deletion and updation of a particular item
    public void showPopUpMenu(View view, int position) {
        ...
        popupMenu.setOnMenuItemClickListener(item -> {
            switch (item.getItemId()) {
                case id.deleteAParticularInformation:
                    ...
                    break;
                case id.updateAParticularInformation:
                    popUpScreenForAddingRecord popUpScreenForAddingRecord = new popUpScreenForAddingRecord();
                    popUpScreenForAddingRecord.setTaskId(task.getTaskId(), true, context, context);
                    popUpScreenForAddingRecord.show(context.getSupportFragmentManager(), popUpScreenForAddingRecord.getTag());
                    break;
            }
            return false;
        });
    ...
    public class TaskViewHolder extends RecyclerView.ViewHolder {
        @BindView(id.recordingDay)
        TextView day;
        @BindView(id.recordingDate)
        TextView date;
        @BindView(id.recordingMonth)
        TextView month;
        @BindView(id.showRecordingTime)
        TextView time;
        ...
        TextView heart_rate;
        @BindView(id.showComment)
        TextView comment;
        @BindView(id.alertOrApprovalImage)
        ImageView image;
        @BindView(id.updateOrDeleteOption)
        ImageView options;
